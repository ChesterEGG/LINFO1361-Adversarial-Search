...
from heuristics.temp import Evaluator
...
class IterativeDeepeningAgent(Agent):
    def __init__(self, player, max_depth=20, custom_weights=None):
        super().__init__(player)
        self.max_depth = max_depth
        self.evaluator = Evaluator(self.player, custom_weights=custom_weights)
    ...
    def act(self, state, remaining_time):
        ...
        self.time_limit = remaining_time / 6.0
        ...
        #print("Ultimate V4: current depth: " + str(self.current_iteration_depth))
    ...
    def alphabeta(self, state, depth, alpha, beta, maximize, extensions=0):
        """
        Alphabeta algorithme avec vérification du temps ignorée jusqu'à depth 6,
        table de transposition et extension singulière
        """
        ...
        extension= 0
        if extensions < 5 and depth > 0:
            # On évalue rapidement le plateau
            score = self.evaluator.evaluate(state, 0)
            if abs(score) >= self.evaluator.extension_threshold:
                # On vérifie que c'est bien un coup forcé, si il y a plus de 6 coup, on ne s'etend pas
                action_count = Game.actions(state)
                if len(action_count) <= 6:
                    extension = 1

        if depth <= 0 or Game.is_terminal(state):
            score = self.quiescence(state, alpha, beta, maximize, depth=0)
            return score
        ...
        if maximize:
            ...
            for action in actions:
                ...
                eval_score = self.alphabeta(next_state, depth - 1 + extension, alpha, beta, False, extensions + extension)
        ...
        # Tour de l'adversaire (Minimize)
            ...
            for action in actions:
                ...
                eval_score = self.alphabeta(next_state, depth - 1 + extension, alpha, beta, True, extensions + extension)
    ...
    def quiescence(self, state, alpha, beta, maximize, depth=0):
        """
        Recherche de Quiescence pour limiter l'Effet d'Horizon.
        Explore uniquement les coups les plus agressifs au-delà de depth=0.
        """
        # 4 coups maximum en cas d'urgence
        if depth > 4 or Game.is_terminal(state):
            return self.evaluator.evaluate(state, 0)
        ...
        if maximize:
            ...
            # Si la position est calme, on s'arrete tout de suite
            if abs(score) < self.evaluator.weight_align_3:
                return score

            acions = Game.actions(state)
            # On prend que les 3 meilleurs coups
            actions = self.sort_action(state, acions, 0)[:3]

            for action in actions:
                next_state = state.copy()
                Game.apply(next_state, action)
                score = self.quiescence(next_state, alpha, beta, False, depth + 1)
            ...
            # Couper alpha immédiatement si la position est trop faible
            if score <= alpha:
                return alpha
            if beta > score:
                beta = score
            ...
            actions = Game.actions(state)
            actions = self.sort_action(state, actions, 0)[:3]

            for action in actions:
                next_state = state.copy()
                Game.apply(next_state, action)
                score = self.quiescence(next_state, alpha, beta, True, depth + 1)
