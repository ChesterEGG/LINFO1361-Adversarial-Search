...
# ICI
            if x_count > 0 and o_count > 0:
                ...
                if x_count == 3 and empty == 1:
                    # 1. On enregistre toujours les menaces pour détecter les fourchettes
                    if can_my_player_play_x:
                        my_threats[(empty_row, empty_col)] = my_threats.get((empty_row, empty_col), 0) + 1
                    if can_opp_player_play_x:
                        opp_threats[(empty_row, empty_col)] = opp_threats.get((empty_row, empty_col), 0) + 1

                    # 2. Priorité absolue : Est-ce que quelqu'un va gagner MAINTENANT ?
                    if current_player == self.player and is_reachable and can_my_player_play_x:
                        symbol_score = self.weight_critical
                    elif current_player != self.player and is_reachable and can_opp_player_play_x:
                        symbol_score = -self.weight_critical
                    # 3. Sinon, on donne juste des points de structure
                    else:
                        s_score = 0
                        if can_my_player_play_x:
                ...
                if o_count == 3 and empty == 1:
                    ...
                    if current_player == self.player and is_reachable and can_my_player_play_o:
                        symbol_score = self.weight_critical
                    elif current_player != self.player and is_reachable and can_opp_player_play_o:
                        symbol_score = -self.weight_critical
                    else:
                        s_score = 0
                        if can_my_player_play_o:
